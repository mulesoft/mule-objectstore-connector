 ...
 * LICENSE.txt file.
 */
package org.test.extension.localobjectstore.api;
...
import org.test.extension.localobjectstore.internal.LocalObjectStoreConnector;
import org.test.extension.localobjectstore.internal.ObjectStoreRegistry;
...
public abstract class ExtensionObjectStore implements ObjectStore<Serializable>, Startable, Stoppable, NamedObject {
  ...
  /**
   * A reference to an {@code os-local:config} element which will be used to declare this ObjectStore.
   * <p>
  ...
  @Alias("config-ref")
  @ConfigReference(name = "CONFIG", namespace = "OS-LOCAL")
  @Expression(NOT_SUPPORTED)
  @ParameterDsl(allowInlineDefinition = false)
  protected LocalObjectStoreConnector config;
  ...
  public LocalObjectStoreConnector getConfig() {
    return config;
  }

  public void setConfig(LocalObjectStoreConnector config) {
    this.config = config;
  }
}
